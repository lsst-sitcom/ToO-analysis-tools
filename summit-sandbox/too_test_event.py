...
def main():
    ...
    ipix_disc = hp.query_disc(
        nside=nside, vec=vec, radius=np.radians(16.9)
    )  # This makes a ~900 deg^2 area
    ...
    observatory, scheduler, observations = sim_runner(
        observatory,
        scheduler,
        sim_duration=np.array([5.0]),
        filename=None,
        verbose=True,
    )

    print(
        "Scheduler note: {}".format(
...
def makeFilterDist(observations):
    fig, axs = plt.subplots(
        1,
        len(np.unique(observations[:]["night"])),
        figsize=(5 * len(np.unique(observations[:]["night"])), 5),
        sharey=True,
    )
    for num, ax in zip(np.unique(observations[:]["night"]), axs.flatten()):
        # for num in np.unique(observations[:]["night"]):
        strin = "Exposure times: {}".format(
            np.unique(
                observations["exptime"][[t == num for t in observations[:]["night"]]]
            )
        )
        subset = observations[:][[t == num for t in observations[:]["night"]]]
        ax.scatter(
            subset[:]["mjd"] - 60980 - num, subset[:]["band"], s=subset[:]["exptime"]
        )
        ax.set_title("Night {}\n{}".format(num, strin))
    return fig
...
def makeObsPositions(observations):
    ...
    ax.scatter(
        np.degrees(observations["RA"][too_indx]),
        np.degrees(observations["dec"][too_indx]),
        color="red",
        label="ToO Observations",
    )

    ax.set_xlabel("RA (deg)")
    ax.set_ylabel("Dec (deg)")
    ax.legend()
    ax.set_title("ToO observations for test: Lensed BNS case A")
